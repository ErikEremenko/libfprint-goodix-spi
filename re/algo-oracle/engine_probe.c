...
static unsigned char *auxiliary_bp_address;
static unsigned char auxiliary_bp_original;
static int auxiliary_bp_rearm;
static unsigned auxiliary_bp_calls;
static unsigned char *study_bp_address;
static unsigned char study_bp_original;
static int study_bp_rearm;
static unsigned study_bp_calls;
static unsigned char *scheduler_aux_bp_address;
static unsigned char scheduler_aux_bp_original;
static int scheduler_aux_bp_rearm;
static unsigned scheduler_aux_bp_calls;

static LONG CALLBACK auxiliary_veh(EXCEPTION_POINTERS *ep)
{
    CONTEXT *ctx = ep->ContextRecord;
    DWORD old;

    if (ep->ExceptionRecord->ExceptionCode == EXCEPTION_BREAKPOINT &&
        (ep->ExceptionRecord->ExceptionAddress == auxiliary_bp_address ||
         ep->ExceptionRecord->ExceptionAddress == auxiliary_bp_address + 1)) {
        const int32_t *values = (const int32_t *)(ULONG_PTR)ctx->Rdx;
        const int32_t *config = *(const int32_t **)(ULONG_PTR)(ctx->Rsp + 0x130);
        const int32_t *scratch = (const int32_t *)(ULONG_PTR)ctx->Rsp;

        printf("Chicago live auxiliary[%u] frame=%s values=",
               auxiliary_bp_calls++, fake_retry_raw ? fake_retry_raw : "(none)");
        for (unsigned j = 0; values && j < 6; ++j)
            printf("%s%d", j ? "," : "", values[j]);
        printf(" bytes=");
        for (unsigned j = 0; values && j < 6; ++j)
            printf("%s%u", j ? "," : "", (unsigned)(uint8_t)values[j]);
        printf(" config=");
        for (unsigned j = 0; config && j < 10; ++j)
            printf("%s%d", j ? "," : "", config[j]);
        printf(" counts=%d,%d,%d flag=%lld",
               scratch[0x30 / 4], scratch[0x50 / 4], scratch[0x54 / 4],
               (long long)ctx->Rbp);
        putchar('\n');
        VirtualProtect(auxiliary_bp_address, 1, PAGE_EXECUTE_READWRITE, &old);
        *auxiliary_bp_address = auxiliary_bp_original;
        FlushInstructionCache(GetCurrentProcess(), auxiliary_bp_address, 1);
        ctx->Rip = (DWORD64)(ULONG_PTR)auxiliary_bp_address;
        ctx->EFlags |= 0x100;
        auxiliary_bp_rearm = 1;
        return EXCEPTION_CONTINUE_EXECUTION;
    }
    if (ep->ExceptionRecord->ExceptionCode == EXCEPTION_SINGLE_STEP &&
        auxiliary_bp_rearm) {
        auxiliary_bp_rearm = 0;
        ctx->EFlags &= ~0x100;
        VirtualProtect(auxiliary_bp_address, 1, PAGE_EXECUTE_READWRITE, &old);
        *auxiliary_bp_address = 0xcc;
        FlushInstructionCache(GetCurrentProcess(), auxiliary_bp_address, 1);
        return EXCEPTION_CONTINUE_EXECUTION;
    }
    return EXCEPTION_CONTINUE_SEARCH;
...
/* Re-arm at every type-24 study/admission invocation.  This is deliberately
 * an entry tap: all five non-record inputs have already been produced by the
 * identify scheduler, but +0x251d0 has not yet filtered status or study
 * eligibility. */
static LONG CALLBACK study_veh(EXCEPTION_POINTERS *ep)
{
    CONTEXT *ctx = ep->ContextRecord;
    DWORD old;

    if (ep->ExceptionRecord->ExceptionCode == EXCEPTION_BREAKPOINT &&
        (ep->ExceptionRecord->ExceptionAddress == study_bp_address ||
         ep->ExceptionRecord->ExceptionAddress == study_bp_address + 1)) {
        const int32_t *record = (const int32_t *)(ULONG_PTR)ctx->Rcx;
        const unsigned char *probe =
            (const unsigned char *)(ULONG_PTR)ctx->Rdx;
        const int32_t aggregate =
            *(const int32_t *)(ULONG_PTR)(ctx->Rsp + 0x28);
        const int32_t *status = *(const int32_t **)(ULONG_PTR)(ctx->Rsp + 0x30);
        const int32_t *eligible =
            *(const int32_t **)(ULONG_PTR)(ctx->Rsp + 0x38);
        const int32_t type = *(const int32_t *)(ULONG_PTR)(ctx->Rsp + 0x40);
        const unsigned char *auxiliary = probe ?
            *(const unsigned char *const *)(probe + 0x148) : NULL;

        printf("Chicago study input[%u] frame=%s auxiliary-count=%d "
               "candidate=%d aggregate=%d type=%d status=%d eligible=%d "
               "probe-packed=%d probe-quality=%d probe-density=%d "
               "probe-auxiliary=",
               study_bp_calls++, fake_retry_raw ? fake_retry_raw : "(none)",
               (int32_t)ctx->R8, (int32_t)ctx->R9, aggregate, type,
               status ? *status : -999, eligible ? *eligible : -999,
               probe ? *(const int32_t *)(probe + 0x140) : -999,
               probe ? *(const int32_t *)(probe + 0x10c) : -999,
               probe ? *(const int32_t *)(probe + 0x158) : -999);
        for (unsigned j = 0; j < 6; ++j)
            printf("%s%u", j ? "," : "", auxiliary ? auxiliary[j] : 255);
        printf(" record=");
        for (unsigned j = 0; record && j < 26; ++j)
            printf("%s%d", j ? "," : "", record[j]);
        putchar('\n');

        VirtualProtect(study_bp_address, 1, PAGE_EXECUTE_READWRITE, &old);
        *study_bp_address = study_bp_original;
        FlushInstructionCache(GetCurrentProcess(), study_bp_address, 1);
        ctx->Rip = (DWORD64)(ULONG_PTR)study_bp_address;
        ctx->EFlags |= 0x100;
        study_bp_rearm = 1;
        return EXCEPTION_CONTINUE_EXECUTION;
    }
...
/* +0x29d19 is the join point immediately after the gallery resolution class
 * has been combined with (and, where applicable, folded into) the running
 * probe class.  The locals named here are established by identifytemplate at
 * +0x29820 and were verified against the +0x52bc0 decoder. */
static LONG CALLBACK scheduler_aux_veh(EXCEPTION_POINTERS *ep)
{
    CONTEXT *ctx = ep->ContextRecord;
    DWORD old;

    if (ep->ExceptionRecord->ExceptionCode == EXCEPTION_BREAKPOINT &&
        (ep->ExceptionRecord->ExceptionAddress == scheduler_aux_bp_address ||
         ep->ExceptionRecord->ExceptionAddress == scheduler_aux_bp_address + 1)) {
        const unsigned char *probe =
            (const unsigned char *)(ULONG_PTR)ctx->R13;
        const unsigned char *gallery =
            (const unsigned char *)(ULONG_PTR)ctx->R12;
        const int32_t *frame = (const int32_t *)(ULONG_PTR)ctx->Rbp;

        printf("Chicago scheduler auxiliary[%u] frame=%s "
               "probe-packed=%d gallery-packed=%d gallery-primary=%d "
               "gallery-secondary=%d running=%d combined=%d mode=%lld "
               "prior-rejections=%d raised=%d\n",
               scheduler_aux_bp_calls++,
               fake_retry_raw ? fake_retry_raw : "(none)",
               probe ? *(const int32_t *)(probe + 0x140) : -999,
               gallery ? *(const int32_t *)(gallery + 0x140) : -999,
               frame[0x18c / 4], frame[0x16c / 4], frame[0x160 / 4],
               frame[-0x20 / 4], (long long)ctx->R8,
               (int32_t)ctx->R15, frame[-0x1c / 4]);

        VirtualProtect(scheduler_aux_bp_address, 1, PAGE_EXECUTE_READWRITE, &old);
        *scheduler_aux_bp_address = scheduler_aux_bp_original;
        FlushInstructionCache(GetCurrentProcess(), scheduler_aux_bp_address, 1);
        ctx->Rip = (DWORD64)(ULONG_PTR)scheduler_aux_bp_address;
        ctx->EFlags |= 0x100;
        scheduler_aux_bp_rearm = 1;
        return EXCEPTION_CONTINUE_EXECUTION;
    }
    if (ep->ExceptionRecord->ExceptionCode == EXCEPTION_SINGLE_STEP &&
        scheduler_aux_bp_rearm) {
        scheduler_aux_bp_rearm = 0;
        ctx->EFlags &= ~0x100;
        VirtualProtect(scheduler_aux_bp_address, 1, PAGE_EXECUTE_READWRITE, &old);
        *scheduler_aux_bp_address = 0xcc;
        FlushInstructionCache(GetCurrentProcess(), scheduler_aux_bp_address, 1);
        return EXCEPTION_CONTINUE_EXECUTION;
    }
    return EXCEPTION_CONTINUE_SEARCH;
...
static LONG CALLBACK init_veh(EXCEPTION_POINTERS *ep)
{
    ...
    for (i = 0; i < init_bp_count; ++i) {
        if (init_bps[i].address == hit || init_bps[i].address == previous) {
            ...
            if (!strcmp(init_bps[i].label, "before Chicago auxiliary write")) {
                const int32_t *values = (const int32_t *)(ULONG_PTR)ctx->Rdx;
                printf("  Chicago live auxiliary values=");
                for (unsigned j = 0; values && j < 6; ++j)
                    printf("%s%d", j ? "," : "", values[j]);
                printf(" bytes=");
                for (unsigned j = 0; values && j < 6; ++j)
...
static void trace_chicago_auxiliary(HMODULE algo, void *engine_context)
{
    DWORD old;
    /* +0x137a0 has assembled the six int32 producer values at rsp+0xb0;
     * +0x539d0 immediately truncates them into the live subtemplate +0x148
     * buffer. */
    unsigned char *p = (unsigned char *)algo + 0x13dea;
    traced_engine_context = engine_context;
    AddVectoredExceptionHandler(1, auxiliary_veh);
    if (!VirtualProtect(p, 1, PAGE_EXECUTE_READWRITE, &old)) return;
    auxiliary_bp_address = p;
    auxiliary_bp_original = *p;
    *p = 0xcc;
    VirtualProtect(p, 1, old, &old);
}

static void trace_chicago_study_inputs(HMODULE algo, void *engine_context)
{
    DWORD old;
    unsigned char *p = (unsigned char *)algo + 0x251d0;

    traced_engine_context = engine_context;
    AddVectoredExceptionHandler(1, study_veh);
    if (!VirtualProtect(p, 1, PAGE_EXECUTE_READWRITE, &old)) return;
    study_bp_address = p;
    study_bp_original = *p;
    *p = 0xcc;
...
static void trace_chicago_scheduler_auxiliary(HMODULE algo,
                                               void *engine_context)
{
    DWORD old;
    unsigned char *p = (unsigned char *)algo + 0x29d19;

    traced_engine_context = engine_context;
    AddVectoredExceptionHandler(1, scheduler_aux_veh);
    if (!VirtualProtect(p, 1, PAGE_EXECUTE_READWRITE, &old)) return;
    scheduler_aux_bp_address = p;
    scheduler_aux_bp_original = *p;
    *p = 0xcc;
    VirtualProtect(p, 1, old, &old);
}
                                      ...
                                      const char *probe)
{
    ...
    if (getenv("ENGINE_WARMUP_COUNT"))
        warmup_count = strtoul(getenv("ENGINE_WARMUP_COUNT"), NULL, 0);
    if (warmup_count > gallery_count) {
        printf("invalid ENGINE_WARMUP_COUNT=%zu for %zu captures\n",
               warmup_count, gallery_count);
        return E_INVALIDARG;
    }
    /* fprintd performs duplicate-check identification before starting the
     * driver's enrollment callback.  Those captures are not inserted, but
     * they do advance Chicago's persistent temporal preprocessing state. */
    for (i = 0; i < warmup_count; ++i) {
        hr = engine_accept_one(iface, pipeline, gallery[i], &reject);
        printf("EnrollmentWarmup[%zu] hr=0x%08lx reject=%u\n",
               i + 1, (unsigned long)hr, reject);
        if (FAILED(hr)) return hr;
    }
    gallery += warmup_count;
    gallery_count -= warmup_count;
    ...
    for (i = 0; i < attempts; ++i) {
        ...
        if (FAILED(hr)) {
            /* AcceptSampleData owns preprocessing and therefore advances the
             * persistent temporal state even when its quality/coverage gate
             * rejects this capture.  A real acquisition loop asks for
             * another frame without calling UpdateEnrollment.  Preserve that
             * distinction when replaying complete hardware capture streams. */
            printf("EnrollmentCapture[%zu] admission failed hr=0x%08lx "
                   "reject=%u; UpdateEnrollment skipped\n",
                   i + 1, (unsigned long)hr, reject);
        ...
        printf("UpdateEnrollment[%u] input=%zu hr=0x%08lx reject=%u\n",
               update_attempt, i + 1, (unsigned long)hr, reject);
        if (hr == S_OK) break;
        if (FAILED(hr)) {
...
int main(int argc, char **argv)
{
    ...
    if (getenv("ATTACH")) {
        ...
        if (SUCCEEDED(hr) && pipeline.EngineContext && argc >= 2) {
            ...
            if (getenv("TRACE_AUXILIARY"))
                trace_chicago_auxiliary(GetModuleHandleA("AlgoChicago.dll"), pipeline.EngineContext);
            if (getenv("TRACE_STUDY_INPUTS"))
                trace_chicago_study_inputs(GetModuleHandleA("AlgoChicago.dll"), pipeline.EngineContext);
            if (getenv("TRACE_SCHEDULER_AUX"))
                trace_chicago_scheduler_auxiliary(GetModuleHandleA("AlgoChicago.dll"), pipeline.EngineContext);
